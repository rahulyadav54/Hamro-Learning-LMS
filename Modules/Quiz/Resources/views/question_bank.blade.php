                ...
                <div class="col-lg-4">
                    <div class="row">
                        <div class="col-lg-12">
                            ...
                            <div class="white-box mb-30" style="border: 2px dashed #8b5cf6; border-radius: 12px; background: rgba(139,92,246,0.02); padding: 20px;">
                                <div class="d-flex align-items-center mb-3">
                                    <span class="fas fa-magic" style="font-size: 18px; color: #8b5cf6; margin-right: 10px;"></span>
                                    <h4 class="m-0" style="font-weight: 700; color: #8b5cf6;">AI Question Builder</h4>
                                </div>
                                <p class="text-muted mb-3" style="font-size: 12px; line-height: 1.4;">Select a Group and Category below, enter a topic, and AI will generate 3 MCQ questions instantly.</p>
                                <div class="mb-3">
                                    <input type="text" id="aiQuizTopic" placeholder="e.g. Flutter State Management" class="form-control" style="border-radius: 8px; font-size: 13px; height: 38px;">
                                </div>
                                <button type="button" id="btnAiGenQuestions" class="primary-btn small fix-gr-bg w-100 text-center" style="border-radius: 20px; font-weight: 600;">
...
@push('scripts')
    ...
    $(document).ready(function() {
        $('#btnAiGenQuestions').on('click', function(e) {
            e.preventDefault();
            var topic = $('#aiQuizTopic').val();
            ...
            if(!group || !category) {
                alert('Please select a Group and Category first so the questions are filed correctly!');
                return;
            }
            ...
            $.ajax({
                url: '{{ route("instructor.ai.generate_questions") }}',
                method: 'POST',
                data: {
                    _token: '{{ csrf_token() }}',
                    topic: topic,
                ...
                success: function(res) {
                    btn.prop('disabled', false).html('Generate & Add Questions');
                    if(res.success) {
                        alert(res.message);
                        location.reload();
                    } else {
