    ...
    function send_smtp_mail($config, $receiver_email, $receiver_name, $sender_email, $sender_name, $subject, $message)
    {
        try {
            $transport = (new \Swift_SmtpTransport(
                $config->mail_host,
                $config->mail_port,
                !empty($config->mail_encryption) ? $config->mail_encryption : null
            ))
            ...
            $swiftMessage = (new \Swift_Message($subject))
                ...
                ->setTo([$receiver_email => $receiver_name])
...
function send_php_mail($receiver_email, $receiver_name, $sender_email, $subject, $message)
{
    try {
        ...
        \Illuminate\Support\Facades\Log::error('PHP mail failed: ' . $e->getMessage());
